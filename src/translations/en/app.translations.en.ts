import { AppTranslations } from '../models/app.translations.interface';

export const appTranslationsEN: AppTranslations = {
  project1: {
    title: 'Create a portfolio web page',
    subtitle: 'First personal project',
    descriptions: {
      text1:
        'I created a web page for my portfolio using React. This is the website you are looking at right now.',
      text2:
        'I was able to apply what I had learned so far in practice by making my own from design to implementation and distribution.',
      text3: '',
      text4: '',
      text5: '',
      text6: '',
    },
    list1: {
      title: 'Design Draft',
      detail:
        'https://xd.adobe.com/view/9ca42e05-4621-4844-b961-0a7c3af67053-c9f7/',
    },
    list2: {
      title: 'Main Function',
      detail: 'Responsive webpage, easy to extend with reusable components',
    },
    list3: {
      title: 'GitHub',
      detail: 'https://github.com/hjqueeen/portfolio',
    },
    list4: {
      title: 'Frontend',
      detail: 'React, TypeScript, Sass, Mui, Zustand',
    },
  },
  project2: {
    title: '',
    subtitle: '',
    descriptions: {
      text1: '',
      text2: '',
      text3: '',
      text4: '',
      text5: '',
      text6: '',
    },
    list1: {
      title: '',
      detail: '',
    },
    list2: {
      title: '',
      detail: '',
    },
    list3: {
      title: '',
      detail: '',
    },
  },
  project3: {
    title: '',
    subtitle: '',
    descriptions: {
      text1: '',
      text2: '',
      text3: '',
      text4: '',
      text5: '',
      text6: '',
    },
    list1: {
      title: '',
      detail: '',
    },
    list2: {
      title: '',
      detail: '',
    },
    list3: {
      title: '',
      detail: '',
    },
  },
  intro: {
    title: 'Hyejin Kim',
    subtitle: 'Web Developer Portfolio',
    description: '',
  },
  introduction: {
    title: 'ABOUT ME',
    name: 'Name',
    name_detail: 'Hyejin Kim',
    education: 'Education',
    education_detail:
      'Seoul National University(Department of Biosystem Engineering)',
    email: 'Email',
    email_detail: 'hjqueeen@gmail.com',
    address: 'Address',
    address_detail: 'Berlin, Germany',
    profile: {
      text1: 'Hello. I am a , ',
      text2: 'developer who is still evolving.',
      text3:
        'person who perseveres through difficult tasks and takes responsibility for the tasks assigned to me.',
    },
  },
  skill: {
    title: 'SKILLS',
    frontend: 'Frontend',
    backend: 'Backend',
    version_control: 'Version Control',
    communication: 'Communication',
  },
  archive: {
    title: 'ARCHIVING',
    github: {
      address: 'https://github.com/hjqueeen',
      description: {
        detail: 'This is a source code repository.',
        text1: 'Source code for a personal project',
        text2: 'Code practiced while learning the programming language',
        text3: '',
      },
    },
    website: {
      address: 'https://velog.io/@hjqueeen',
      description: {
        detail: 'This blog is for study and knowledge sharing purposes.',
        text1: 'Records to make what I studied truly my own',
        text2: 'Collection of programming knowledge learned as a developer',
        text3: '',
      },
    },
  },
  title: "HJK's Portfolio",
};
